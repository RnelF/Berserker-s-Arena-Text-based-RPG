...
let PlayerQuests = [
  {
    ...
    description:
      "The nearby Grassland village is being terrorized by ferocious mutant Rabbits called Rabituzah. Eliminate the Rabituzahs to restore peace.",
...
const monsterRandomPicker = () => {
  ...
  let availableMonsters = valleyMonstersList.filter(
    (monster) =>
      monster.monsterLevel >= minLevel && monster.monsterLevel <= maxLevel
  );
...
const calculatePlayerDamage = (
  ...
  let damageOutput = parseInt(
    totalPlayerStr / strDivisor + totalPlayerAgi / agiDivisor - monsterDef / 4
  );
...
const calculateMonsterDamage = (
  ...
  let damageOutput = parseInt(
    monsterStrength / strDivisor +
      monsterAgility / agiDivisor -
      playerDefense / 4
  );
...
const calculateMonsterCritChance = (
  monsterAgility,
  monsterLck,
  playerAgility,
  playerLck
  ...
  const monsterCritChance = parseInt(
    monsterAgility + monsterLck - (playerAgility + playerLck) / 4
  );
...
const updateMonsterStatsUI = (monster) => {
  ...
  const fightingMonsterHealth = document.getElementById(
    "fighting-monster-health"
  );
  const fightingMonsterLevel = document.getElementById(
    "fighting-monster-level"
  );
  const fightingMonsterImg = document.getElementById("fighting-monster-img");
  const fightingMonsterEnergy = document.getElementById(
    "fighting-monster-energy"
  );
  ...
  fightingMonsterHealth.textContent =
    monster.health <= 0
      ? 0
      : monster.health; /*added ternary operator so it would display negative digits*/
...
const updatePlayerStatsUI = () => {
  ...
  currentWeaponUI.textContent = playerCurrentWeapon
    ? playerCurrentWeapon.weaponName
    : "No Weapon Equipped";
  currentHelmetUI.textContent = playerCurrentHelmet
    ? playerCurrentHelmet.armorName
    : "No Helmet Equipped";
  currentArmorUI.textContent = playerCurrentArmor
    ? playerCurrentArmor.armorName
    : "No Armor Equipped";
  currentGauntletUI.textContent = playerCurrentGauntlet
    ? playerCurrentGauntlet.armorName
    : "No Gauntlet Equipped";
  currentBootsUI.textContent = playerCurrentBoots
    ? playerCurrentBoots.armorName
    : "No Boots Equipped";
  currentShieldUI.textContent = playerCurrentShield
    ? playerCurrentShield.armorName
    : "No Shield Equipped";
...
const useHealthPotion = () => {
  ...
  if (healthPotionIndex !== -1) {
    ...
    appendUseHPFightStory(
      `You used HP potion and Regenerated ${healthPotionHealAmount} HP`
    );
  } else {
...
const useEnergyPotion = () => {
  ...
  if (energyPotionIndex !== -1) {
    ...
    appendUseEnergyFightStory(
      `You used Energy potion and Regenerated ${energyPotionRestoreAmount} Energy`
    );
  } else {
...
const valleyMonstersFightFunction = () => {
  ...
  const playerAction = () => {
    ...
    const playerDamage = calculatePlayerDamage(
      ...
      playerAgi,
      playerCurrentWeapon,
      selectedMonster.monsterDef
    );
    const monsterEvade = checkEvasion(
      selectedMonster.monsterAgi,
      selectedMonster.monsterLuck
    );
    ...
    if (monsterEvade) {
      ...
      appendFightStoryPlayerMove(
        `You used basic attack and hit ${selectedMonster.name} for ${playerDamage} damage!`
      );
      updateMonsterStatsUI(selectedMonster);
    }
  ...
  const monsterAction = () => {
    ...
    const monsterDamage = calculateMonsterDamage(
      selectedMonster.monsterStr,
      selectedMonster.monsterAgi,
      playerDef
    );
    const playerEvade = checkEvasion(playerAgi, playerLuck);
    const monsterCriticalChance = calculateMonsterCritChance(
      selectedMonster.monsterAgi,
      selectedMonster.monsterLuck,
      playerAgi,
      playerLuck
    ...
    if (playerEvade) {
      appendFightStoryMonsterMove(
        `You evaded ${selectedMonster.name}'s attack!`
      );
    } else {
  ...
  const handleCriticalAttack = (skill, monsterDamage) => {
    if (selectedMonster.monsterEnergy >= skill.skillEnergyConsumption) {
      if (skill.skillName === "stun" || skill.skillName === "earthSmash") {
        const monsterSkillDamage = parseInt(
          skill.skillDmg * 2 <= 0 ? 2 : skill.skillDmg * 2
        );

        appendCritDmgFightStory(
          `${selectedMonster.name} uses ${skill.skillName} CRITICAL!! It deals ${monsterSkillDamage} damage! ${selectedMonster.name} takes another turn!`
        );
        ...
        appendFightStoryMonsterMove(
          `${selectedMonster.name} uses ${skill.skillName} and gained ${additionalDef} additional Defense!`
        );
        ...
        appendCritDmgFightStory(
          `${selectedMonster.name} uses ${skill.skillName} CRITICAL!! It deals ${monsterSkillDamage} damage!`
        );
        selectedMonster.monsterEnergy -= skill.skillEnergyConsumption;
      ...
      const monsterCritDmg = parseInt(
        monsterDamage * 2 <= 0 ? 2 : monsterDamage * 2
      );
      playerHealth -= monsterCritDmg;
      appendCritDmgFightStory(
        `${selectedMonster.name} doesn't have enough energy left, uses basic attack CRITICAL!! Deals ${monsterCritDmg} damage!`
      );
    }
  ...
  const handleNormalAttack = (skill, monsterDamage) => {
    if (selectedMonster.monsterEnergy >= skill.skillEnergyConsumption) {
      if (skill.skillName === "stun" || skill.skillName === "earthSmash") {
        const monsterSkillDamage = parseInt(
          skill.skillDmg - playerDef / 3 < 0
            ? 1
            : skill.skillDmg - playerDef / 3
        );

        appendFightStoryMonsterMove(
          `${selectedMonster.name} uses ${skill.skillName} for ${skill.skillEnergyConsumption} energy! It deals ${monsterSkillDamage} damage!\n you missed your turn!`
        );
        selectedMonster.monsterEnergy -= skill.skillEnergyConsumption;
      ...
      appendFightStoryMonsterMove(
        `${selectedMonster.name} doesn't have enough energy left, uses basic attack and hits you for ${monsterDamage} damage!`
      );
    }
  };
...
const currentWeaponContainer = document.getElementById(
  "current-weapon-container"
);
const townInventoryContainer = document.getElementById(
...
const displayWeaponInventoryUI = () => {
  ...
  if (playerWeaponInv.length === 0) {
    ...
    playerWeaponInv.forEach((weapon) => {
      ...
      let isEquipped =
        playerCurrentWeapon &&
        playerCurrentWeapon.weaponName === weapon.weaponName;
...
const removeWeaponFromInventory = (weaponName) => {
  const weaponIndex = playerWeaponInv.findIndex(
    ...
    playerArmorInv.forEach((armor) => {
      ...
      switch (armor.armorName) {
        case "BreastPlate":
          isEquipped =
            playerCurrentArmor &&
            playerCurrentArmor.armorName === armor.armorName;
          break;
        case "Gauntlet":
          isEquipped =
            playerCurrentGauntlet &&
            playerCurrentGauntlet.armorName === armor.armorName;
          break;
        case "Armored Boots":
          isEquipped =
            playerCurrentBoots &&
            playerCurrentBoots.armorName === armor.armorName;
          break;
        case "Great Helm":
          isEquipped =
            playerCurrentHelmet &&
            playerCurrentHelmet.armorName === armor.armorName;
          break;
        case "Round Shield":
          isEquipped =
            playerCurrentShield &&
            playerCurrentShield.armorName === armor.armorName;
          break;
      }
...
const equipWeapon = (weaponName) => {
  ...
  if (weapon) {
    ...
    weaponName === "crossbow"
      ? (playerLuck += weapon.weaponBonusAtt)
...
const darkValleyBtn = document
  .getElementById("dark-valley-btn")
...
const backButton = document
  .getElementById("notif-back-button")
  .addEventListener("click", () => {
...
const valleyBtn = document
  .getElementById("valley-btn")
  .addEventListener("click", valleyFunction);
...
const blacksmithStoreBackBtn = document.getElementById(
  "blacksmith-store-back-btn"
);
const blacksmithStoreNotif = document.getElementById(
  "blacksmith-store-notif-container"
);
const blacksmithStoreNotifTxt = document.getElementById(
  "blacksmith-store-notif"
);
const blacksmithStoreNotifBckBtn = document.getElementById(
  "blacksmith-store-notif-bck-btn"
);

const bsBuyConfirmation = document.getElementById(
  "blacksmith-store-confirmation-container"
);
const bsConfirmationTxt = document.getElementById(
  "blacksmith-store-confirmation-txt"
);
const bsConfirmationYesBtn = document.getElementById(
  "blacksmith-confirm-yes-btn"
);
const bsConfirmationNoBtn = document.getElementById(
  "blacksmith-confirm-no-btn"
);
...
const breastplateCloseInfoBtn = document.getElementById(
  "breastplate-close-info"
);
const breastplateInfoContainer = document.getElementById(
  "breastplate-info-container"
);
...
const gauntletInfoContainer = document.getElementById(
  "gauntlet-info-container"
);
...
const swordsmithStoreBackBtn = document.getElementById(
  "swordsmith-store-back-btn"
);

const swordsmithStoreNotif = document.getElementById(
  "swordsmith-store-notif-container"
);
const swordsmithStoreNotifTxt = document.getElementById(
  "swordsmith-store-notif"
);
const swordsmithStoreNotifBckBtn = document.getElementById(
  "swordsmith-store-notif-bck-btn"
);

const smBuyConfirmation = document.getElementById(
  "swordsmith-store-confirmation-container"
);
const smConfirmationTxt = document.getElementById(
  "swordsmith-store-confirmation-txt"
);
const smConfirmationYesBtn = document.getElementById(
  "swordsmith-confirm-yes-btn"
);
const smConfirmationNoBtn = document.getElementById(
  "swordsmith-confirm-no-btn"
);
...
const crossbowInfoContainer = document.getElementById(
  "crossbow-info-container"
);
...
const nunchuksInfoContainer = document.getElementById(
  "nunchuks-info-container"
);
...
const alchemistStoreBackBtn = document.getElementById(
  "alchemist-store-back-btn"
);
...
const alchemyStoreNotif = document.getElementById(
  "alchemy-store-notif-container"
);
const alchemyStoreNotifTxt = document.getElementById("alchemy-store-notif");
const alchemyStoreNotifBckBtn = document.getElementById(
  "alchemy-store-notif-bck-btn"
);
...
const buyHealthPotionMinusBtn = document.getElementById(
  "buy-health-potion-minus"
);
const healthPotionQuantityInput = document.getElementById(
  "health-potion-quantity"
);
const buyHealthPotionPlusBtn = document.getElementById(
  "buy-health-potion-plus"
);
const healthPotionConfirmBtn = document.getElementById("hp-confirm-btn");

const buyEnergyPotionMinusBtn = document.getElementById(
  "buy-energy-potion-minus"
);
const energyPotionQuantityInput = document.getElementById(
  "energy-potion-quantity"
);
const buyEnergyPotionPlusBtn = document.getElementById(
  "buy-energy-potion-plus"
);
const energyPotionConfirmBtn = document.getElementById("energy-confirm-btn");

const buyConfirmation = document.getElementById(
  "alchemist-store-confirmation-container"
);
const confirmationTxt = document.getElementById(
  "alchemy-store-confirmation-txt"
);
  ...
  if (hpQuantity > 0) {
    if (playerGold >= healthPotionCost * hpQuantity) {
      showConfirmationDialog(hpQuantity, "Health", healthPotionCost, () => {
        playerGold -=
          hpQuantity > 0 ? healthPotionCost * hpQuantity : healthPotionCost;
  ...
  if (eQuantity > 0) {
    if (playerGold >= energyPotionCost * eQuantity) {
      showConfirmationDialog(eQuantity, "Energy", energyPotionCost, () => {
        playerGold -=
          eQuantity > 0 ? energyPotionCost * eQuantity : energyPotionCost;
...
breastplateBuyBtn.addEventListener("click", () => {
  showBsConfirmationDialog(
    armors.breastplate.armorName,
    armors.breastplate.armorCost,
    () => {
      if (playerGold >= armors.breastplate.armorCost) {
        ...
        blacksmithStoreNotifTxt.textContent =
          "You successfully bought a Breastplate";

        updatePlayerStatsUI();
      } else {
        blacksmithStoreNotif.style.display = "inline-block";
        blacksmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Breastplate";
      }
    }
  );
});
...
gauntletBuyBtn.addEventListener("click", () => {
  showBsConfirmationDialog(
    armors.gauntlet.armorName,
    armors.gauntlet.armorCost,
    () => {
      if (playerGold >= armors.gauntlet.armorCost) {
        ...
        blacksmithStoreNotifTxt.textContent =
          "You successfully bought a Gauntlet";

        updatePlayerStatsUI();
      } else {
        blacksmithStoreNotif.style.display = "inline-block";
        blacksmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Gauntlet";
      }
    }
  );
});
...
bootsBuyBtn.addEventListener("click", () => {
  showBsConfirmationDialog(
    armors.armoredBoots.armorName,
    armors.armoredBoots.armorCost,
    () => {
      if (playerGold >= armors.armoredBoots.armorCost) {
        ...
        blacksmithStoreNotifTxt.textContent =
          "You successfully bought a Armored Boots";

        updatePlayerStatsUI();
      } else {
        blacksmithStoreNotif.style.display = "inline-block";
        blacksmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Armored Boots";
      }
    }
  );
});
...
helmetBuyBtn.addEventListener("click", () => {
  showBsConfirmationDialog(
    armors.greatHelm.armorName,
    armors.greatHelm.armorCost,
    () => {
      if (playerGold >= armors.greatHelm.armorCost) {
        ...
        blacksmithStoreNotifTxt.textContent =
          "You successfully bought a Great Helm";

        updatePlayerStatsUI();
      } else {
        blacksmithStoreNotif.style.display = "inline-block";
        blacksmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Great Helm";
      }
    }
  );
});
...
shieldBuyBtn.addEventListener("click", () => {
  showBsConfirmationDialog(
    armors.shield.armorName,
    armors.shield.armorCost,
    () => {
      if (playerGold >= armors.shield.armorCost) {
        ...
        blacksmithStoreNotifTxt.textContent =
          "You successfully bought a Shield";

        updatePlayerStatsUI();
      } else {
        blacksmithStoreNotif.style.display = "inline-block";
        blacksmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Shield";
      }
    }
  );
});
...
swordBuyBtn.addEventListener("click", () => {
  showSmConfirmationDialog(
    weapons.sword.weaponName,
    weapons.sword.weaponCost,
    () => {
      if (playerGold >= weapons.sword.weaponCost) {
        ...
        swordsmithStoreNotif.style.display = "inline-block";
        swordsmithStoreNotifTxt.textContent =
...
//buy dagger functions
const buyDagger = () => {
  swordContainer.style.display = "none";
  daggerContainer.style.display = "flex";
  crossbowContainer.style.display = "none";
  katanaContainer.style.display = "none";
  nunchuksContainer.style.display = "none";
};

daggerBuyBtn.addEventListener("click", () => {
  showSmConfirmationDialog(
    weapons.dagger.weaponName,
    weapons.dagger.weaponCost,
    () => {
      if (playerGold >= weapons.dagger.weaponCost) {
        playerGold -= weapons.dagger.weaponCost;

        swordsmithStoreNotif.style.display = "inline-block";
        playerWeaponInv.push(weapons.dagger);
        swordsmithStoreNotifTxt.textContent =
          "You successfully bought a Dagger";

        updatePlayerStatsUI();
      } else {
        swordsmithStoreNotif.style.display = "inline-block";
        swordsmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Dagger";
      }
    }
  );
});

daggerCancelBtn.addEventListener("click", () => {
  daggerContainer.style.display = "none";
});

daggerInfoBtn.addEventListener("click", () => {
  daggerInfoContainer.style.height = "100%";
  daggerInfoBtn.style.display = "none";
});

daggerCloseInfoBtn.addEventListener("click", () => {
  daggerInfoContainer.style.height = "0"; // Collapse info container
...
crossbowBuyBtn.addEventListener("click", () => {
  showSmConfirmationDialog(
    weapons.crossbow.weaponName,
    weapons.crossbow.weaponCost,
    () => {
      if (playerGold >= weapons.crossbow.weaponCost) {
        ...
        swordsmithStoreNotifTxt.textContent =
          "You successfully bought a Crossbow";

        updatePlayerStatsUI();
      } else {
        swordsmithStoreNotif.style.display = "inline-block";
        swordsmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Crossbow";
      }
    }
  );
});
...
katanaBuyBtn.addEventListener("click", () => {
  showSmConfirmationDialog(
    weapons.katana.weaponName,
    weapons.katana.weaponCost,
    () => {
      if (playerGold >= weapons.katana.weaponCost) {
        ...
        swordsmithStoreNotifTxt.textContent =
          "You successfully bought a Katana";

        updatePlayerStatsUI();
      } else {
        swordsmithStoreNotif.style.display = "inline-block";
        swordsmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a Katana";
      }
    }
  );
});
...
nunchuksBuyBtn.addEventListener("click", () => {
  showSmConfirmationDialog(
    weapons.nunchuks.weaponName,
    weapons.nunchuks.weaponCost,
    () => {
      if (playerGold >= weapons.nunchuks.weaponCost) {
        ...
        swordsmithStoreNotifTxt.textContent =
          "You successfully bought a NunChuks";

        updatePlayerStatsUI();
      } else {
        swordsmithStoreNotif.style.display = "inline-block";
        swordsmithStoreNotifTxt.textContent =
          "You don't have enough gold to buy a NunChuks";
      }
    }
  );
});
...
function showConfirmationDialog(quantity, potionType, potionCost, onConfirm) {
  ...
  confirmationTxt.textContent = `Are you sure you want to buy ${quantity} ${potionType} Potion(s) for ${
    potionCost * quantity
  } gold?`;
...
const townBtn = document
  .getElementById("town-btn")
  .addEventListener("click", townBtnFunction);
